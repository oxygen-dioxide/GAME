#!/usr/bin/env python3
"""
Memory-optimized full pipeline benchmark script for GAME ONNX inference.
Enforces a maximum chunk length to control VRAM usage.
"""
...
def enforce_max_chunk_size(chunks, max_duration_s, samplerate):
...
def run_mem_optimized_benchmark():
    ...
    print(f"✓ Max Chunk Duration: {max_chunk_duration_s}s")
    ...
    # --- Enforce Max Chunk Size for VRAM Control ---
    processed_chunks = enforce_max_chunk_size(chunks, max_chunk_duration_s, samplerate)
    timer.a_lap(f"Chunk Size Enforcement (New total: {len(processed_chunks)} chunks)")
    ...
    for i, chunk in enumerate(processed_chunks):
        ...
        print(f"  - Chunk {i+1}/{len(processed_chunks)} (len: {chunk_duration[0]:.2f}s) processed in {loop_elapsed:.2f} ms, found {valid.sum()} notes.")

    timer.stages['Inference (Total)'] = total_inference_time
    
    print("\n" + "="*60)
    print("Memory-Optimized Benchmark Summary")
    print("="*60)
