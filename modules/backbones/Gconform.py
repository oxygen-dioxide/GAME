...
from .base_conv import conform_conv
...
class Gcf(nn.Module):
    def __init__(self, dim: int, kernel_size: int = 31, conv_drop: float = 0.1, ffn_latent_drop: float = 0.1,
                 ...
                 attention_heads_dim: int = 64):
        super().__init__()
        self.att1 = conform_blocke(dim=dim, kernel_size=kernel_size, conv_drop=conv_drop,
                                   ffn_latent_drop=ffn_latent_drop,
                                   ffn_out_drop=ffn_out_drop, attention_drop=attention_drop,
                                   attention_heads=attention_heads,
                                   attention_heads_dim=attention_heads_dim)
        self.att2 = conform_blocke(dim=dim, kernel_size=kernel_size, conv_drop=conv_drop,
                                   ffn_latent_drop=ffn_latent_drop,
                                   ffn_out_drop=ffn_out_drop, attention_drop=attention_drop,
                                   attention_heads=attention_heads,
                                   attention_heads_dim=attention_heads_dim)
...
class Gmidi_conform(nn.Module):
    def __init__(self, indim: int, outdim: int, lay: int, dim: int, use_lay_skip: bool, kernel_size: int = 31,
                 conv_drop: float = 0.1,
